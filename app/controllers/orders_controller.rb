class OrdersController < MerchantApplicationController
  ...
  def create
      ...
      selling_stock = params[:product_summary]
      order_summary = params[:order_summary]
    ...
    @current_order.merchant_account_id = order_summary['merchant_account_id']
    @current_order.customer_id=1
    @current_order.name = orders_bill_code(order_summary['warehouse_id'])
    @current_order.return=0
    @current_order.delivery=1
    ...
    #   (ví dụ có 2 đợt nhập cùng sản phẩm - và số lượng bán vượt qua một đợt thì sẽ phải trừ cả 2 đợt để cho đủ số sản phẩm)
      ...
      subtract_quality_on_sale stocking_items, item, @current_order, @current_order.delivery
    ...
    #   2.Cập nhật các số liệu khác: tổng tiền, giảm giá...
      #TODO chưa trừ khi discount_cash lớn hơn 0
      @current_order.discount_cash += item['discount_cash']
      @current_order.total_price += item['price'] * item['quality']
  ...
  private
    ...
    def selling_check_quality_before_sale(product_array)
      product_id=[]
      product_array.each { |item| product_id += [item['ids']] }
       @Product_Summary = ProductSummary.where(id:product_id.uniq)
      product_array.each { |item|
        if @Product_Summary.find(item['ids']).quality < item['quality'] #item.buying_quality
    ...
    def subtract_quality_on_sale (stocking_items, selling_item, current_order, is_delivery_sale)
      ...
      stocking_items.each do |stock|
        ...
        OrderDetail.create!(
            ...
            :discount_cash=>selling_item['discount_cash'])
      ...
      stocking_item = @Product_Summary.find(selling_item['ids'])
      stocking_item.quality -= selling_item['quality']
      stocking_item.save

      #Chỉ tru so luong khi is_delivery_sale = true
      metro_summary.stock_count -= selling_item['quality'] if is_delivery_sale == false
      metro_summary.sale_count += selling_item['quality'] if is_delivery_sale == false
      metro_summary.sale_count_day += selling_item['quality'] if is_delivery_sale == false
      metro_summary.sale_count_month += selling_item['quality'] if is_delivery_sale == false
